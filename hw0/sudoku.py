#sudoku.py
#Boyang Zhang and Nathan Fraenkel
...
class SudokuBoard:
    ...
    def ParseBoard(self, name):
        ...
        for x in range(len(k)):
            board.append([int(b) for b in k[x]])
    ...
    def computeUnusedNums(self, constraint): #constraint will be a set of 9 tuples containing board locations
	...
	for s in self.__constraints:
	    if self.computeUnusedNums(s) != set([]): #computeUnusedNums will return an empty set if all spaces in a constraint contain a number
		solved = False
		break
	return solved

    def dfsSuccessor(self, board, q):
    ...
    def DFS(self):
        ...
        while(not q.empty()):
            if(self.isSolved()):
                return self.board
            self.board = q.get()
            self.dfsSuccessor(self.board, q)
